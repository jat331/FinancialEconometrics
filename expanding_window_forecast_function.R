expanding_window = function(y, init_win_len, pre_sel_num_lags=NA, num_step_ahead = 1,sel_method = 'aic'){
    ...
    if(num_lags==0){
      ...
      for (j in 1:num_step_ahead) {
        if (i >= j) {
          if (j <= num_lags){
            if (j==1){
              x_f = cbind(1,t(y_adj[(trian_idx-num_lags+1):trian_idx]))
            } else {
              x_f = cbind(1,t(as.matrix(yhat_f[i,1:(j-1)])),t(as.matrix(y_adj[(trian_idx-num_lags+j):trian_idx])))
            }
          } else {
